    ...
    <div class="p-datatable-wrapper" :style="{ maxHeight: scrollHeight }">
      <table ref="table" role="table" class="p-datatable-table">
        ...
        <DTTableBody
          ...
          @editing-cell-change="onEditingCellChange($event)"
        />
        <DTTableBody
          :value="dataToRender"
          :columns="columns"
...
export default {
  ...
  updated() {
    ...
    if (
      this.scrollable &&
      (this.scrollDirection !== "vertical" || this.rowGroupMode === "subheader")
    ) {
      this.updateScrollWidth();
    }
  },
  methods: {
    columnProp(col, prop) {
      return col.props
        ? col.type.props[prop].type === Boolean && col.props[prop] === ""
          ? true
          : col.props[prop]
        : null;
    },
    ...
    toggleRowsWithCheckbox(event) {
      ...
      const _selection = checked
        ? []
        : this.frozenValue
        ? [...this.frozenValue, ...processedData]
        : processedData;
      this.$emit("update:selection", _selection);

      if (checked) this.$emit("row-unselect-all", { originalEvent: event });
      else
    ...
    onEditingCellChange(event) {
      ...
      if (editing) _editingCells.push({ rowIndex, cellIndex });
      else
        _editingCells = _editingCells.filter(
          (cell) =>
    ...
    updateScrollWidth() {
      let parentElementHeight = DomHandler.width(
        this.$refs.table.parentElement
      );
      ...
      if (this.$refs.table.scrollWidth > parentElementHeight) {
        ...
        this.$refs.table.style.width =
          parentElementHeight - DomHandler.calculateScrollbarWidth() + "px";
      }
    },
